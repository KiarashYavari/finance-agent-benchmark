...
from tools.yfinance_search import get_financial_metrics, get_financial_ratios, get_ticker_symbol
    ...
    def _register_mcp_tools(self):
        ...
        async def sec_search_handler(
            ...
            try:
                ...
                print(f"[GREEN] Fetching {company_name} ticker={ticker_symbol} start={start_date} end={end_date}")
                
                # Look for ticker symbol, if none was passed as parameter
                if ticker_symbol == None:
                    result = get_ticker_symbol_handler(company_name)
                    wticker = result.get("ticker")
                    if len(wticker != 0):
                        ticker_symbol = wticker
                        print(f"[GREEN] Found ticker={wticker}")
                        
                # Search for filings
                search_result = await sec_search(
